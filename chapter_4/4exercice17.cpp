using namespace std;
#include "std_lib_facilities.h"


int main () {
    cout << "Please give a suite of strings, and let us return you the mode of it. "<<endl;
    vector<string> values;
    vector<string> unique_values;// The idea is to have a vector that has elements corresponding to frenquencies.
    vector<int> times;// frequencies

    string i="error";
    
    while (cin >> i){
        values.push_back(i);
    }
if (values.size() != 0) {

    sort( values.begin(),values.end());
    unique_values.push_back(values[0]);
    times.push_back(1);
    for (int i =1;i<values.size();i++){
         if (values[i]==values[i-1]){// we increment of 1 the frequency of the current value in our (unique) times vector
            times[times.size()-1]++;
         }else {
            unique_values.push_back(values[i]);// We encountered a new item in values
            times.push_back(1);}
            }

    int max_freq=0;
    string mode="error?";
    for (int i =0; i<unique_values.size();i++){
        if (times[i]>max_freq)  {
            max_freq=times[i];
            mode=unique_values[i];
        }

    }
    cout << " mode is : "<< mode << endl;

}
else cout << "empty";

    return 0;}
