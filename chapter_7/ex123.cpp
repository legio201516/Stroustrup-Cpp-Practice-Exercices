...
double primary()
{
	...
	case name_type:{
		...
		else return get_value(t.name);// or if no affectation , just give the variable value.
		break;}
...
bool is_declared(string var){// is var already declared ? 
    for (int i=0; i<var_table.size(); i++){
...
//handles name= expression, declares a variable named "name" with initial value "expression"
	...
	double d = expression();
	define_name(var_name, d, k==let);// we define a new name with bool that indicates if const or variable
	return d;
}
...
try {
    define_name("pi", 3.141592, false);     // We first define names
    define_name("e", 2.71828, false);
